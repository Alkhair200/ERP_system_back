...
class JobsController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = Jobs::with(['admin' => function($q){
            return $q->select('id','name');
        }])->where('com_code',$com_code)->orderBy('created_at','desc')->paginate(PAGINATION_COUNT);

        return $this->returnData('jobs', $data , 'تم ارسال البيانات بنجاح'); 
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $com_code = Auth()->user()->com_code;
            $data = Jobs::where(['com_code' => $com_code ,'name' => $request->name])->first();
            ...
            Jobs::create($request->except(['token']));
    ...
    public function update(Request $request ,$id)
    {
        ...
        try {
            $com_code = Auth()->user()->com_code;
            $data = Jobs::where(['com_code' => $com_code ,'id' => $id])->first();
